...
def sample() -> Bundle:
    ...
    # `field.characteristic` first, because a real bundle has it first and
    # because `check.order-unique` rests on it. Without this claim the
    # fixture could not validate at all: the dependency table has required
    # it since round two, and this bundle was still shaped the way it was
    # before that. A fixture that cannot pass `validate()` is a fixture
    # that stopped describing the format.
    bundle.add_claim(
        "field.characteristic",
        {"p": canonical.as_str(SECP_P), "prime": "proved"},
        "proof.ecpp",
        {"subject": canonical.as_str(SECP_P), "steps": []},
    )
    bundle.add_claim(
        "curve.order.prime",
...
def test_tier_comes_from_evidence_not_from_the_producer():
    ...
    assert bundle.tier_counts() == {"A": 3, "D": 1, "X": 0}
...
def test_proof_may_not_rest_on_a_candidate():
    ...
    # Located by name rather than by index. It used to be `claims[1]`,
    # which is the kind of thing that keeps working while pointing at
    # something else: add a claim to the fixture and the test starts
    # replacing the wrong one, still raises, and still reads as green.
    position = next(
        index for index, record in enumerate(bundle.claims)
        if record.claim == "curve.order"
    )
    bundle.claims[position] = Claim(
        "curve.order",
        bundle.claims[position].asserts,
        "check.order-unique",
        bundle.claims[position].evidence_ref,
