...
def test_both_policy_engines_agree(exported, tmp_path):
    ...
    # `as_uri()`, not `as_posix()`. On Windows an absolute path is
    # `E:/EK/web/src/policy.js`, and Node's ESM loader reads the leading
    # `E:` as a URL scheme and refuses it outright. A `file://` URL is
    # what the loader documents and it is correct on both platforms, so
    # this is not a Windows workaround but the spelling that was always
    # right. The test could not run on Windows at all until now.
    script = tmp_path / "run.mjs"
    script.write_text(
        f'''
import {{ readFileSync }} from "node:fs";
import {{ evaluate }} from "{POLICY_JS.as_uri()}";
