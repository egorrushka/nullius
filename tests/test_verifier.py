...
def test_verifier_stays_readable():
    """The size budget is a feature, and it has been raised twice, both times
    for arithmetic rather than for convenience. Each raise should be an
    argument, not a reflex."""
    sizes = {
    ...
    assert sum(sizes.values()) < 1800, f"verifier grew to {sum(sizes.values())} lines"
