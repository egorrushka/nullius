...
SKIP_FILES = {
    "corpus.generated.js",
    # Built by tools\build_wasm.bat, or copied from the placeholder beside
    # it when there is no wasm toolchain. Either way regenerated, and the
    # built form is half a megabyte of base64 that nobody can read,
    # review or diff — and that could disagree with the Rust it claims to
    # be without anyone seeing.
    "verifier.generated.js",
}

SKIP_PATHS = {
    Path("corpus"),
    Path("web/public/data"),
    # The compiled verifier and its binding. Regenerated by
    # tools\build_wasm.bat from the Rust beside it, so a copy here would
    # be a half-megabyte of base64 nobody can read, review or diff — and
    # one that could silently disagree with the source it claims to be.
    Path("web/src/wasm"),
}
